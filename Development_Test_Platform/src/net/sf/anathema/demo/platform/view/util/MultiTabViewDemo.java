...
public class MultiTabViewDemo extends SwingDemoCase {
  ...
  private MultiTabView createDemoView() {
    ...
    new SimpleViewTabContent(new ContentProperties("First Scroll Needed").needsScrollbar(), new DemoTabView(true)).addTo(view); //$NON-NLS-1$
    new SimpleViewTabContent(new ContentProperties("Second Scroll Needed").needsScrollbar(), new DemoTabView(true)).addTo(view); //$NON-NLS-1$
    new SimpleViewTabContent(new ContentProperties("First No Scroll Needed"), new DemoTabView(false)).addTo(view); //$NON-NLS-1$
    new SimpleViewTabContent(new ContentProperties("Second No Scroll Needed"), new DemoTabView(false)).addTo(view); //$NON-NLS-1$
    view.initGui(null);
    return view;
  }
}