/*
 * Copyright 2000-2003 The Apache Software Foundation Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * @version $Id: AbstractElement.java,v 1.6 2005/09/30 11:01:24 vestrial Exp $
 */
public abstract class AbstractElement extends AbstractParentChildNode implements Element {
   ...
   * @exception DOMException INVALID_CHARACTER_ERR: if name contains invalid characters,
   */
  protected AbstractElement(String name, AbstractDocument owner) {
    ownerDocument = owner;
    if (!DOMUtilities.isValidName(name)) {
      throw createDOMException(DOMException.INVALID_CHARACTER_ERR, "xml.name", new Object[] { name }); //$NON-NLS-1$
    }
  }
  ...
  public String getAttribute(String name) {
    ...
    return (attr == null) ? "" : attr.getValue(); //$NON-NLS-1$
  }
  ...
  public Attr removeAttributeNode(Attr oldAttr) throws DOMException {
    ...
    if (attributes == null) {
      throw createDOMException(DOMException.NOT_FOUND_ERR, "attribute.missing", new Object[] { oldAttr.getName() }); //$NON-NLS-1$
    }
    String nsURI = oldAttr.getNamespaceURI();
    return (Attr) attributes.removeNamedItemNS(nsURI, (nsURI == null ? oldAttr.getNodeName() : oldAttr.getLocalName()));
  }
  ...
  /**
   * <b>DOM</b>: Implements {@link org.w3c.dom.Element#hasAttributeNS(String,String)}.
   */
  ...
  /**
   * <b>DOM</b>: Implements {@link org.w3c.dom.Element#removeAttributeNS(String,String)}.
   */
  ...
  /**
   * <b>DOM</b>: Implements {@link org.w3c.dom.Element#getAttributeNodeNS(String,String)}.
   */
    ...
    for (Node n = this; n != null; n = n.getParentNode()) {
      switch (n.getNodeType()) {
        ...
        case DOCUMENT_NODE:
          ...
          l = ad.getElementsByTagName(n, "*"); //$NON-NLS-1$
          if (l != null) {
            l.invalidate();
          }
          ...
          lns = ad.getElementsByTagNameNS(n, "*", ln); //$NON-NLS-1$
          if (lns != null) {
            lns.invalidate();
          }
          lns = ad.getElementsByTagNameNS(n, ns, "*"); //$NON-NLS-1$
          if (lns != null) {
            lns.invalidate();
          }
          lns = ad.getElementsByTagNameNS(n, "*", "*"); //$NON-NLS-1$ //$NON-NLS-2$
          if (lns != null) {
            lns.invalidate();
          }
      }
  ...
  protected void checkChildType(Node n, boolean replace) {
    switch (n.getNodeType()) {
      ...
      case DOCUMENT_FRAGMENT_NODE:
        break;
      default:
        throw createDOMException(DOMException.HIERARCHY_REQUEST_ERR, "child.type", new Object[] { //$NON-NLS-1$
            new Integer(getNodeType()), getNodeName(), new Integer(n.getNodeType()), n.getNodeName() });
    }
  }
  ...
  public void fireDOMAttrModifiedEvent(String name, Attr node, String oldv, String newv, short change) {
    ...
    if (doc.getEventsEnabled() && !oldv.equals(newv)) {
      DocumentEvent de = doc;
      MutationEvent ev = (MutationEvent) de.createEvent("MutationEvents"); //$NON-NLS-1$
      ev.initMutationEvent("DOMAttrModified", true, // canBubbleArg //$NON-NLS-1$
          false, // cancelableArg
          node, // relatedNodeArg
          oldv, // prevValueArg
          newv, // newValueArg
   ...
   */
  protected abstract void attrAdded(Attr node, String newv);

  /**
   * Called when an attribute has been modified.
   */
  protected abstract void attrModified(Attr node, String oldv, String newv);

  /**
   * Called when an attribute has been removed.
   */
  protected abstract void attrRemoved(Attr node, String oldv);
  ...
  public class NamedNodeHashMap implements NamedNodeMap, Serializable {
    ...
    /**
     * <b>DOM</b>: Implements {@link org.w3c.dom.NamedNodeMap#getNamedItemNS(String,String)}.
     */
    ...
    public Node removeNamedItemNS(String namespaceURI, String localName) throws DOMException {
      if (isReadonly()) {
        throw createDOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR, "readonly.node.map", new Object[] {}); //$NON-NLS-1$
      }
      if (localName == null) {
        throw createDOMException(DOMException.NOT_FOUND_ERR, "attribute.missing", new Object[] { "" }); //$NON-NLS-1$ //$NON-NLS-2$
      }
      AbstractAttr n = (AbstractAttr) remove(namespaceURI, localName);
      if (n == null) {
        throw createDOMException(DOMException.NOT_FOUND_ERR, "attribute.missing", new Object[] { localName }); //$NON-NLS-1$
      }
      n.setOwnerElement(null);

      // Mutation event
      fireDOMAttrModifiedEvent(n.getNodeName(), n, n.getNodeValue(), "", MutationEvent.REMOVAL); //$NON-NLS-1$
      return n;
    }
    ...
    public Node setNamedItem(String ns, String name, Node arg) throws DOMException {
      ...
      if (result != null) {
        ...
        fireDOMAttrModifiedEvent(name, result, result.getNodeValue(), "", MutationEvent.REMOVAL); //$NON-NLS-1$
      }
      fireDOMAttrModifiedEvent(name, (Attr) arg, "", arg.getNodeValue(), MutationEvent.ADDITION); //$NON-NLS-1$
      return result;
    }
    ...
    protected void checkNode(Node arg) {
      ...
      if (getOwnerDocument() != arg.getOwnerDocument()) {
        throw createDOMException(DOMException.WRONG_DOCUMENT_ERR, "node.from.wrong.document", new Object[] { //$NON-NLS-1$
            new Integer(arg.getNodeType()), arg.getNodeName() });
      }
      if (arg.getNodeType() == ATTRIBUTE_NODE && ((Attr) arg).getOwnerElement() != null) {
        throw createDOMException(DOMException.WRONG_DOCUMENT_ERR, "inuse.attribute", new Object[] { arg.getNodeName() }); //$NON-NLS-1$
      }
    }
