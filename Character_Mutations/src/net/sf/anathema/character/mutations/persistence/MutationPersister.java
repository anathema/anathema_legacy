...
public class MutationPersister extends AbstractQualityPersister<IMutation> {
  ...
  private void loadMutation(IQualityModel<IMutation> model, Element element)
  	throws PersistenceException
  {
	  String giftId = element.attributeValue(ATTRIB_ID);
      //backward compatability
      giftId.replaceAll("DeadlyBeastmanTransformation.Gift",
    		  "Mutations.Mutation");
      ...
      final boolean experienceActive = ElementUtilities.getBooleanAttribute(
          ...
          false);
      model.addQualitySelection(new QualitySelection<IMutation>(
          mutationModel.getMutationById(giftId),
          value,
          creationActive,
          experienceActive));
  }
