...
public class AbyssalAppearanceMinimum extends ReflectionEqualsObject implements IExtendedMinimum {
  ...
  public boolean isFullfilledWithout(ILimitationContext context, ITraitType traitType) {
    int currentEssence = context.getTraitCollection().getTrait(OtherTraitType.Essence).getCurrentValue();
    int currentAppearance = context.getTraitCollection().
            getTrait(AttributeType.Appearance).getCurrentValue();
    if (currentEssence == 4) {
      if (currentAppearance <= 2) currentMinimum = 1;
      else currentMinimum = 3;
      return false;
    } else if (currentEssence >= 5) {
      if (currentAppearance < 2) currentMinimum = 0;
      else currentMinimum = 4;
      return false;
    }
    currentMinimum = 1;
