...
public abstract class AbstractFxContainerPanel extends AbstractFxPane {
	
	private final SelectionViewFactory selectionViewFactory;
	
	protected AbstractFxContainerPanel(SelectionViewFactory selectionViewFactory, 
			final LC layout, final AC col, final AC row) {
	...
	public <T> ObjectSelectionView<T> addSelectionView(String label, AgnosticUIConfiguration<T> ui, final CC constraints) {
	...
	public <T extends AbstractFxPane> T addSubpanel(final T subpanel, final String title, final CC constraints) {
		Platform.runLater(new Runnable() {
		      @Override
		      public void run() {
