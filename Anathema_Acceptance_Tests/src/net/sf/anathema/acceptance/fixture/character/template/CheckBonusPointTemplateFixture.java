...
public class CheckBonusPointTemplateFixture extends
		AbstractTemplateColumnFixture {
	...
	public int getFavoredAbilityPointCost() {
		return getTemplate().getBonusPointCosts().getAbilityCosts(true)
				.getRatingCosts(1);
	}

	public int getUnfavoredAbilityPointCost() {
		return getTemplate().getBonusPointCosts().getAbilityCosts(false)
				.getRatingCosts(1);
	}

	public int getUnfavoredSpecialtyDotsPerPoint() {
		return getTemplate().getBonusPointCosts()
				.getDefaultSpecialtyDotsPerPoint();
	}

	public int getFavoredSpecialtyDotsPerPoint() {
		return getTemplate().getBonusPointCosts()
				.getFavoredSpecialtyDotsPerPoint();
	}

	public int getLowBackgroundCost() {
		return getTemplate().getBonusPointCosts().getBackgroundBonusPointCost()
				.getRatingCosts(1);
	}
	...
	public int getVirtueCost() {
		return getTemplate().getBonusPointCosts().getVirtueCosts()
				.getRatingCosts(1);
	}
