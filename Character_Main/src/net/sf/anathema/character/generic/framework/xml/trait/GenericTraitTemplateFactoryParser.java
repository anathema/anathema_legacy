...
public class GenericTraitTemplateFactoryParser extends AbstractXmlTemplateParser<GenericTraitTemplateFactory> {
  ...
  private void parseBackgrounds(Element element, GenericTraitTemplateFactory templateFactory) throws PersistenceException {
    Element backgroundsElement = element.element(TAG_BACKGROUNDS);
    if (backgroundsElement == null) {
      return;
    }
    GenericTraitTemplatePoolParser traitTemplatePoolParser =
            new GenericTraitTemplatePoolParser(poolTemplateRegistry, new BackgroundTraitGroup(backgroundRegistry));
