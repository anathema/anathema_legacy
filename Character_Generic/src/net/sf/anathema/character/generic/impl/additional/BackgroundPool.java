...
public class BackgroundPool implements IAdditionalEssencePool {
  ...
  public BackgroundPool(IBackgroundTemplate template, AdditionalEssencePool personalPool, AdditionalEssencePool peripheralPool,
                        ComplexAdditionalEssencePool[] complexPools, boolean modifiesBase) {
  ...
  public IdentifiedInteger[] getAdditionalComplexPools(IGenericTraitCollection traitCollection, IMagicCollection magicCollection) {
    ...
    for (int i = 0; i < complexPools.length; i++) {
      poolValues[i] = new IdentifiedInteger(complexPools[i].getId(), complexPools[i].getPool(traitCollection, getCurrentValue(traitCollection)));
    }
    return poolValues;
  }
