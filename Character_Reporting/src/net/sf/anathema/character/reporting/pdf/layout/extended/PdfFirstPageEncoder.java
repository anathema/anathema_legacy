...
public class PdfFirstPageEncoder implements IPdfPageEncoder {
 ...
 encodeCombatStats(graphics, character, distanceFromTop, remainingHeight);*/
  ...
  private void encodeCopyright(SheetGraphics graphics) throws DocumentException {
    ...
    graphics.createSimpleColumn(firstColumnBounds).withLeading((float) lineHeight).andTextPart(voidstatePhrase).encode();
    ...
    graphics.createSimpleColumn(anathemaBounds).withLeading(lineHeight).andAlignment(Center).andTextPart(anathemaPhrase).encode();
    ...
    graphics.createSimpleColumn(whitewolfBounds).withLeading(lineHeight).andAlignment(Right).andTextPart(whitewolfPhrase).encode();
  }
