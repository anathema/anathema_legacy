...
public class NewPdfMagicPageEncoder extends AbstractPdfPageEncoder {
  ...
  public void encode(Document document, PdfContentByte directContent, ReportContent content) throws DocumentException {
    ...
    // Essence & Willpower
    float essenceHeight = encodeEssence(directContent, content, distanceFromTop, getContentHeight() - distanceFromTop);
    float willpowerHeight = encodeWillpower(directContent, content, distanceFromTop, 96.625f);
    ...
    if (needsMagic) {
      ...
      encodeMagic(directContent, content, distanceFromTop, getContentHeight() - distanceFromTop);

      // Left-hand side: Magic Sidebars (e.g. Thaumaturgical Degrees), Initiations, Notes
      float sidebarHeight = encodeSidebars(directContent, content, distanceFromTop, getContentHeight() - distanceFromTop);
      if (sidebarHeight != 0) {
        distanceFromTop += calculateBoxIncrement(sidebarHeight);
      }
      float initiationHeight = encodeInitiations(directContent, content, distanceFromTop, getContentHeight() - distanceFromTop);
      if (initiationHeight != 0) {
        distanceFromTop += calculateBoxIncrement(initiationHeight);
      }
      encodeNotes(directContent, content, "EnlightenmentNotes", 1, 1, distanceFromTop, getContentHeight() - distanceFromTop, 1);
    ...
    if (charmTemplate.canLearnCharms(character.getRules())) {
      ...
      float animaHeight = 0;
      if (hasAnima()) {
        animaHeight = encodeAnima(directContent, content, distanceFromTop, 128f);
      }
      ...
      if (character.getTemplate().getEdition() == ExaltedEdition.SecondEdition) {
        float genericCharmsHeight = encodeGenericCharms(directContent, content, distanceFromTop, getContentHeight() - distanceFromTop);
      ...
      encodeCharms(directContent, content, printCharms, distanceFromTop, remainingHeight);
      while (!printCharms.isEmpty()) {
        encodeCopyright(directContent);
        document.newPage();
        encodeCharms(directContent, content, printCharms, 0, getPageConfiguration().getContentHeight());
      }
      encodeCopyright(directContent);
    }
  }
  ...
  private float encodeEssence(PdfContentByte directContent, ReportContent content, float distanceFromTop, float height) throws DocumentException {
    return encodeVariableBox(directContent, content, (IVariableBoxContentEncoder) getPartEncoder().getEssenceEncoder(), 1, 2, distanceFromTop,
      height);
  }
  ...
  private float encodeCombos(PdfContentByte directContent, IGenericCharacter character, float distanceFromTop, float fixedHeight,
  ...
  private float encodeAnima(PdfContentByte directContent, ReportContent content, float distanceFromTop, float height) throws DocumentException {
    return encodeFixedBox(directContent, content, getPartEncoder().getAnimaEncoder(), 3, 1, distanceFromTop, height);
  }

  private float encodeGenericCharms(PdfContentByte directContent, ReportContent content, float distanceFromTop,
    ...
    if (encoder.hasContent(content)) {
      return encodeVariableBox(directContent, content, encoder, 1, 3, distanceFromTop, maxHeight);
    }
    else {
      return 0f;
    }
  ...
  private float encodeMagic(PdfContentByte directContent, ReportContent content, float distanceFromTop, float height) throws DocumentException {
    return encodeFixedBox(directContent, content, new PdfMagicEncoder(getResources(), getBaseFont(), PdfMagicEncoder.collectPrintSpells(content),
      getRegistry().getAdditionalMagicEncoders(), true, "Magic"), //$NON-NLS-1$
      2, 2, distanceFromTop, height);
  }

  private float encodeSidebars(PdfContentByte directContent, ReportContent content, float distanceFromTop,
    float maxHeight) throws DocumentException {
    float height = 0;
    for (IVariableBoxContentEncoder sidebar : getRegistry().getAdditionalMagicSidebarEncoders()) {
      float sidebarHeight = encodeVariableBox(directContent, content, sidebar, 1, 1, distanceFromTop, maxHeight - height);
      if (sidebarHeight != 0) {
  ...
  private float encodeInitiations(PdfContentByte directContent, ReportContent content, float distanceFromTop,
    float maxHeight) throws DocumentException {
    return encodeVariableBox(directContent, content, new PdfInitiationEncoder(getResources(), getBaseFont()), 1, 1, distanceFromTop, maxHeight);
  }

  private float encodeCharms(PdfContentByte directContent, ReportContent content, List<IMagicStats> printCharms, float distanceFromTop,
    float height) throws DocumentException {
    return encodeFixedBox(directContent, content, new PdfMagicEncoder(getResources(), getBaseFont(), printCharms), 1, 3, distanceFromTop, height);
  }
