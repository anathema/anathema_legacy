...
<xsl:stylesheet xmlns:chrm="http://anathema.sourceforge.net/charms" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
        ...
        <xsl:element name="html">
            ...
            <xsl:element name="head">
                <xsl:element name="title">You are reviewing:
                    <xsl:choose>
                        <xsl:when test="chrm:genericCharm!=''">
                            <xsl:value-of select="chrm:genericCharm/@exalt"/>
                            Excellencies
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="chrm:charm/@group"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:element>
            </xsl:element>
            ...
            <xsl:element name="body">
                ...
                <xsl:element name="p">There are
                    <xsl:value-of select="count(chrm:genericCharm)+count(chrm:charm)"/>
                    charms in<xsl:value-of select="comment()"/>.
                </xsl:element>
                <xsl:element name="hr"/>
    ...
    <xsl:template match="chrm:alternatives">
        <xsl:element name="div">
            ...
            <xsl:element name="br"/>
            <xsl:element name="p">
                <xsl:element name="b">Alternative Charms:</xsl:element>
    ...
    <xsl:template name="Line_1">
        <xsl:element name="p">
            <xsl:element name="b">Name:</xsl:element>
            <xsl:value-of select="@id"/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template name="Line_2">
        <xsl:element name="p">
            <xsl:element name="b">Cost:</xsl:element>
            <xsl:for-each select="chrm:cost">
                <xsl:call-template name="charmCost"/>
            </xsl:for-each>
            ;
            ...
            Essence
            <xsl:value-of select="chrm:prerequisite/chrm:essence/@value"/>
        </xsl:element>
    </xsl:template>

    <xsl:template name="Line_3">
        <xsl:element name="p">
            <xsl:element name="b">Type:</xsl:element>
            <xsl:for-each select="chrm:charmtype">
                <xsl:choose>
                    <xsl:when test="@type='Simple'">
                        <xsl:text>Simple</xsl:text>
                        <xsl:if test="chrm:special/@*!=''">
                            <xsl:text>(</xsl:text>
                            <xsl:if test="chrm:special/@speed!=6">
                                <xsl:value-of select="concat('Speed ',chrm:special/@speed)"/>
                            </xsl:if>
                            ...
                            <xsl:if test="chrm:special/@speed!=6">
                                <xsl:if test="chrm:special/@defense!=-1">,</xsl:if>
                            </xsl:if>
                            <xsl:if test="chrm:special/@defense!=-1">DV
                                <xsl:value-of select="chrm:special/@defense"/>
                            </xsl:if>
                            <xsl:text>)</xsl:text>
                        </xsl:if>
                    </xsl:when>
                    <xsl:when test="@type='Reflexive'">
                        ...
                        <xsl:if test="chrm:special/@primaryStep!=''">
                            ...
                            <xsl:if test="chrm:special/@secondaryStep!=''">
                                ...
                                <xsl:value-of select="chrm:special/@secondaryStep"/>
                            </xsl:if>
                            )
                        </xsl:if>
                    </xsl:when>
    ...
    <xsl:template name="Line_4">
        <xsl:element name="p">
            <xsl:element name="b">Keywords:</xsl:element>
    ...
    <xsl:template name="Line_5">
        <xsl:element name="p">
            <xsl:element name="b">Duration:</xsl:element>
            <xsl:choose>
                <xsl:when test="chrm:duration/@amount!=''">
                    <xsl:value-of select="chrm:duration/@amount"/>
                    <xsl:text></xsl:text>
                    <xsl:value-of select="chrm:duration/@unit"/>
                </xsl:when>
                <xsl:when test="chrm:duration/@event!=''">
                    Until
                    <xsl:value-of select="chrm:duration/@event"/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template name="Line_6">
        <xsl:element name="p">
            <xsl:element name="b">Prerequisite Charms:</xsl:element>
        ...
        <xsl:choose>
            <xsl:when test="*">
                <xsl:if test="boolean(number(chrm:essence/@cost))='true'">
                    <xsl:if test="chrm:essence/@cost!=''">
                        <xsl:value-of select="concat(chrm:essence/@cost,'m')"/>
                    </xsl:if>
                </xsl:if>
                <xsl:if test="boolean(number(chrm:essence/@cost))=false">
                    <xsl:if test="chrm:essence/@cost!=''">
                        <xsl:value-of select="chrm:essence/@cost"/>
                    </xsl:if>
                </xsl:if>
                <xsl:if test="chrm:essence/@text!=''">
                    <xsl:text></xsl:text>
                    <xsl:value-of select="chrm:essence/@text"/>
                </xsl:if>
                <xsl:if test="chrm:willpower/@cost!=''">,
                    <xsl:value-of select="concat(chrm:willpower/@cost,'wp')"/>
                </xsl:if>
                <xsl:if test="chrm:willpower/@text!=''">
                    <xsl:text></xsl:text>
                    <xsl:value-of select="chrm:willpower/@text"/>
                </xsl:if>
                <xsl:if test="chrm:health/@cost!=''">,
                    <xsl:value-of select="concat(chrm:health/@cost,'lhl')"/>
                </xsl:if>
                <xsl:if test="chrm:health/@text!=''">
                    <xsl:text></xsl:text>
                    <xsl:value-of select="chrm:health/@text"/>
                </xsl:if>
                <xsl:if test="chrm:experience/@cost!=''">,
                    <xsl:value-of select="concat(chrm:experience/@cost,'xp')"/>
                </xsl:if>
                <xsl:if test="chrm:experience/@text!=''">
                    <xsl:text></xsl:text>
                    <xsl:value-of select="chrm:experience/@text"/>
                </xsl:if>
            </xsl:when>
    ...
    <xsl:template name="charmGroup">
        <xsl:if test="@group!=''">
            <xsl:element name="p">
                <xsl:element name="b">Group:</xsl:element>
    ...
    <xsl:template name="charmRestrictions">
        <xsl:element name="p">
            <xsl:element name="b">Restrictions:</xsl:element>
    ...
    <xsl:template name="charmSource">
        <xsl:element name="p">
            <xsl:element name="b">Source:</xsl:element>
            <xsl:for-each select="chrm:source">
                <xsl:value-of select="@source"/>
                <xsl:if test="@page!=''">on page
    ...
    <xsl:template name="hiddenKeywords">
        <xsl:element name="p">
            <xsl:element name="b">Hidden Keywords:</xsl:element>
    ...
    <xsl:template name="writeCharmComments">
        <xsl:element name="table">
            <xsl:element name="tr">
                <xsl:element name="td">
                    <xsl:element name="b">Comment:</xsl:element>
                </xsl:element>
    ...
    <xsl:template name="writeExalt">
        <xsl:element name="p">
            <xsl:element name="b">Exalt:</xsl:element>
            <xsl:value-of select="@exalt"/>
        </xsl:element>
    </xsl:template>
