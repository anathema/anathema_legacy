...
public class SecondEditionMutationModel extends AbstractMutationsModel {
  ...
  public SecondEditionMutationModel(final ICharacterModelContext context, IBeastformModel model) {
    super(context);
    this.allMutations = MutationProvider.getMutations(new DBTMutationRules());
    model.addCharmLearnCountChangedListener(new IIntValueChangedListener() {
      @Override
      public void valueChanged(int newValue) {
        SecondEditionMutationModel.this.maxPicks = newValue == 0 ? 0 : 4 + (hasTBA(
                context) ? 2 : 1) * context.getTraitCollection().getTrait(OtherTraitType.Essence).getCurrentValue();
        updateOverview();
      }
    });
  }
  ...
  private boolean hasTBA(ICharacterModelContext context) {
    for (ICharm charm : context.getCharmContext().getCharmConfiguration().getLearnedCharms())
      if (charm.getId().equals(TERRIFYING_BEASTMAN_ALTERATION)) return true;
    return false;
  }

  @Override
  public boolean isSelectable(IMutation quality) {
    return super.isSelectable(quality) && getGroupCost(getSelectedQualities()) + quality.getCost() <= maxPicks;
  }
